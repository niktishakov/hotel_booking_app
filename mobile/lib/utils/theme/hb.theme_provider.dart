...
import 'hb.theme.dart';

/// Note this provider does not use the standard [BaseNotifier] found in the mobile package in order to minimise dependencies
class HbThemeProvider extends ChangeNotifier {
  HbThemeProvider(this._hbTheme);

  bool _mounted = true;

  HbTheme _hbTheme;
  HbTheme get hbTheme => _hbTheme;

  @deprecated
  void setNewTheme(HbTheme theme) {
    _hbTheme = theme;
  }
...
extension HbThemeBuildContextExtension on BuildContext {
  HbTheme get hbTheme {
    return Provider.of<HbThemeProvider>(this, listen: false)._hbTheme;
  }

  void changeTheme(HbTheme? value) {
    if (value != null) {
      final hbThemeProvider = Provider.of<HbThemeProvider>(this, listen: false);
      hbThemeProvider._hbTheme = value;
      hbThemeProvider.notifyListeners();
    }
  }
